# 目录

## JavaScript系列文章

1. [JavaScript正则表达式](./articles/Javascript正则表达式.md)
2. [JavaScript数据结构](./articles/JavaScript数据结构.md)
3. [JavaScript隐式类型转换](./articles/JavaScript隐式类型转换.md)
4. [JavaScript模块化规范](./articles/JavaScript模块化规范.md)

## 其他文章

1. [CommitMessage规范校验及版本自动发布](./articles/CommitMessage规范校验及版本自动发布.md)

## 代码片段

1. [装饰者模式](./demos/decorator.js)
2. [单例模式](./demos/singleton.js)
3. [对象的继承方式](./demos/inherit.js)
4. [函数柯里化](./demos/curry.js)
5. [深度优先遍历与广度优先遍历](./demos/DFS&BFS.js)
6. [Vue事件派发与广播](./demos/emitter.js)
7. [迭代器设计](./demos/iterator.js)
8. [手写new](./demos/new.js)
9. [手写promise](./demos/promise.js)
10. [手写set](./demos/set.js)
11. [Vue组件查找](./demos/vue-find-component.js)
