...
- 当使用 require 命令加载同一个模块时，不会再执行该代码，而是取到缓存中的值。也就是说，CommonJS 模块只会在第一次加载时运行一次，以后再加载，只会返回运行的结果，除非手动清除系统缓存。
- 循环加载时，属于加载时执行。即在 require 的时候，模块就会执行。一旦某个模块被循环加载，只会输出已执行的部分，未执行的部分不会输出
